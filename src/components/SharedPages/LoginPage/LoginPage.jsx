    ...
    const onSubmitHandler = (e) => {
        ...
        setTimeout(() => {
            if (username.toLowerCase() !== "admin" && password.toLowerCase() !== "admin") {
                toast.error(<FormattedMessage defaultMessage="To continue fill this inputs with the word: admin!" id="LoginError" />, { autoClose: 2500, hideProgressBar: true, closeOnClick: true, pauseOnFocusLoss: true, theme: 'colored', })
        ...
        <div className="loginP w-100 d-flex ai-center jc-center w-100 h-100">
            <div className={`loginP-Content ${selectedUser && 'user-checked'}`}>
                <h2 className="loginP-Title t-center">
                    <FormattedMessage defaultMessage="Choose Account Type" id="LoginP.Title" />
                </h2>
                ...
                <div className={`loginP-Wrapper ${selectedUser && 'user-checked'}`}>
                    <div className='loginP-Form p-relative'>
                        <h4 className='t-center'>
                            <FormattedMessage defaultMessage="Hello our dear " id="LoginP.SubTitle" />
                            {selectedUser !== null && (<> <FormattedMessage defaultMessage={`${selectedUser}`} id={`${selectedUser}`} />!</>)}
                        </h4>
                        <h6 className='t-center'>
                            <FormattedMessage sdefaultMessage="Please fill out the form below to get started!" id="LoginP.Paragraph" />
                        </h6>
                        <form onSubmit={onSubmitHandler}>
                            ...
                            <Input
                                name="Password"
                                type="password"
                                label={<FormattedMessage defaultMessage="Password" id="Password" />}
                                icon={<FiLock />}
                                onChange={(e) => setPassword(e.target.value)}
                            />
                            <div className='loginP-Form-Buttons d-flex ai-center jc-space_between'>
                                <Link to="/reset-password"><FormattedMessage defaultMessage="Forgot password ?" id="Forgot" /></Link>
                                <Button text={<FormattedMessage defaultMessage="Login" id="Login" />} />
                            </div>
                        </form>
                    </div>
                    <div className='loginP-Footer d-flex ai-center jc-space_between'>
                        <div className='loginP-Footer-Left'>
                            <p><FormattedMessage defaultMessage="Need Help ?" id="NeedHelp" /></p>
                            <Link to="/need-help"><FormattedMessage defaultMessage="Contact Us" id="ContactUs" /></Link>
                        </div>
                        <div className='loginP-Footer-Right'>
                            <p><FormattedMessage defaultMessage="No Account ?" id="NoAccount" /></p>
                            <Link to="/sign-up"><FormattedMessage defaultMessage="Sign Up" id="SignUp" /></Link>
                        </div>
                    </div>
