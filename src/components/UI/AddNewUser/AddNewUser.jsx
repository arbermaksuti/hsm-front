...
const AddNewUser = ({ inputsData }) => {
    ...
    const onAddNewHandler = (showModal) => {
        if (input === "") {
            toast.error(
                <FormattedMessage
                    defaultMessage="You should select on option!"
                    id="SelectOneOption"
                />,
                {
                    autoClose: 2000,
    ...
    return inputsData ? (
        <div className="addNewUser borderedDiv d-flex ai-center jc-space_between">
            <div className="addNewUser-Wrapper">
                <h3>
                    {inputsData.length === 1 ? (
                        `Here you can add new account for ${input}`
                    ) : (
                        <FormattedMessage
                            defaultMessage="Choose type of account which you want to add"
                            id="AddNewTitle"
                        />
                    )}
                </h3>
                    ...
                    {inputsData.map((i, index) => (
                        <div className="addNewUser-Input c-pointer" key={index}>
                            <input
                                ...
                                name="addNew"
                                value={i.type}
                                onChange={(e) => onInputSelectHandler(e)}
                                className="c-pointer"
                            />
                            <label htmlFor={i.type} className="c-pointer">
                                {i.type}
                            </label>
                        </div>
                    ))}
            ...
            <Button
                type="submit"
                form="addNewForm"
                onClick={() => onAddNewHandler(showModal)}
